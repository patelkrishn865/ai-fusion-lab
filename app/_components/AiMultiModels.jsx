  ...
  return (
    <div className="flex flex-1 h-[65vh] border-b">
      {aiModelList.map((model, index) => (
        <div key={index} className={`flex flex-col border overflow-auto h-full ${model.enable ? 'flex-1 min-w-100' : 'w-25 flex-none'}`}>
        <div className="flex w-full h-17.5 items-center justify-between border-b p-4">
          ...
          <div>
            {model.enable ? <Switch checked={model.enable} onCheckedChange={(v) => onToggleChange(model.model, v)} /> : <MessageSquare onClick={() => onToggleChange(model.model, true)} />}
          </div>
          </div>
